...
//      m_debugDraw.DrawPolygon(v, m_polygonA.m_count, color1);
      m_world.Draw.DrawPolygon(v, m_polygonA.m_count, color1);

      for i := 0 to m_polygonB.m_count - 1 do
         v[i] := b2Mul(m_transformB, m_polygonB.m_vertices[i]);
//      m_debugDraw.DrawPolygon(v, m_polygonB.m_count, color1);
      m_world.Draw.DrawPolygon(v, m_polygonB.m_count, color1);

   end;

//   m_debugDrawerOpenGL.DrawPoint(output.pointA, 4.0, color2);
   m_debugDrawer.DrawPoint(output.pointA, 4.0, color2);

//   m_debugDrawerOpenGL.DrawPoint(output.pointB, 4.0, color3);
   m_debugDrawer.DrawPoint(output.pointB, 4.0, color3);
