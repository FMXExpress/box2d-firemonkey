...
//      m_debugDraw.DrawPolygon(v, m_polygonA.m_count, color);
      m_world.Draw.DrawPolygon(v, m_polygonA.m_count, color);

      for i := 0 to m_polygonB.m_count - 1 do
         v[i] := b2Mul(m_transformB, m_polygonB.m_vertices[i]);
//      m_debugDraw.DrawPolygon(v, m_polygonB.m_count, color);
      m_world.Draw.DrawPolygon(v, m_polygonB.m_count, color);
   end;

   for i := 0 to manifold.pointCount - 1 do
//      m_debugDrawerOpenGL.DrawPoint(worldManifold.points[i], 4.0, color2);
      m_debugDrawer.DrawPoint(worldManifold.points[i], 4.0, color2);
