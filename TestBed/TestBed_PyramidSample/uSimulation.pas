...
uses
  UPhysics2DTypes, UPhysics2D, uDebugDrawer, FMX.Objects;
...
type
  ...
  TTestClass = class of TSimulation;

  TSimulation = class(Tb2ContactListener)
  protected
  ...
  public
    ...
    procedure SetGUIForm(const Value: IGUIForm);
...
procedure TSimulation.NextLine;
begin
  if m_InvertedY then
...
procedure TSimulation.Step(var settings: TSettings; timeStep: PhysicsFloat);
const
   k_impulseScale = 0.1;
   k_axisScale = 0.4;
...
procedure TSimulation.Keyboard(key: Byte);
begin
end;

procedure TSimulation.KeyboardUp(key: Byte);
begin
end;
...
procedure TSimulation.MouseDown(const p: TVector2);
const
   k_maxCount = 10;
var
   aabb: Tb2AABB;
...
procedure TSimulation.ShiftMouseDown(const p: TVector2);
begin
  m_mouseWorld := p;
  if Assigned(m_mouseJoint) then
    Exit;
...
procedure TSimulation.MouseUp(const p: TVector2);
begin
   if Assigned(m_mouseJoint) then
   begin
...
procedure TSimulation.MouseMove(const p: TVector2);
begin
   m_mouseWorld := p;
	 if Assigned(m_mouseJoint) then
...
procedure TSimulation.LaunchBomb(velocity_factor: PhysicsFloat = 1.0);
var
   p, v: TVector2;
begin
   {$IFDEF OP_OVERLOAD}
...
procedure TSimulation.LaunchBomb(const position, velocity: TVector2);
var
   bd: Tb2BodyDef;
...
procedure TSimulation.SpawnBomb(const worldPt: TVector2);
begin
  m_bombSpawnPoint := worldPt;
  m_bombSpawning := True;
end;

procedure TSimulation.CompleteBombSpawn(const p: TVector2);
const
  multiplier = 30.0;
var
  vel: TVector2;
...
procedure TSimulation.SetCanvasTranslation(x, y: PhysicsFloat);
begin
  if Assigned(m_debugDrawer) then
    m_debugDrawer.SetCanvasTranslation(x,y);
end;

procedure TSimulation.SetCanvasTranslationOffset(dx, dy: PhysicsFloat);
begin
  if Assigned(m_debugDrawer) then
    m_debugDrawer.SetCanvasTranslationOffset(dx,dy);
end;

procedure TSimulation.SetDebugDrawer(const Value: TDebugDrawer);
begin
  m_debugDrawer := Value;
  m_world.Draw := Value;
end;

procedure TSimulation.SetGUIForm(const Value: IGUIForm);
begin
  FGUIForm := Value;
end;

procedure TSimulation.DrawText(const text: string);
begin
  if m_DrawDebugInfo then
  begin
    if Assigned(m_debugDrawer) then
...
procedure TSimulation.JointDestroyed(joint: Tb2Joint);
begin
end;

procedure TSimulation.PreSolve(var contact: Tb2Contact; const oldManifold: Tb2Manifold);
var
  i: Integer;
...
procedure TSimulation.ShiftOrigin(const newOrigin: TVector2);
begin
